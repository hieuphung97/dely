...
| --- | --- | --- | --- |
| Cursor sidecar shares `./skills/`, no third version | `tests/contracts.sh` on `.cursor-plugin/plugin.json` | `"skills": "./.cursor/skills/"` or a `"version"` field while README claims support | `bash tests/contracts.sh /tmp/fixture-t5a` → `FAIL: .cursor-plugin/plugin.json .skills must be ./skills/ (got ./.cursor/skills/)` line 76; `bash tests/contracts.sh /tmp/fixture-t5b` → `FAIL: .cursor-plugin/plugin.json must not have a version field` line 77 |
| Root Antigravity/Grok manifest unchanged | existing root `.name == "dely"` check; live `agy plugin validate .` and `grok plugin validate .` still ok | root `plugin.json` gained `$schema` or extra keys | `agy plugin validate .` → `[ok]` skills 2 processed; `grok plugin validate .` → `Plugin manifest is valid.` |
| Setup discovers Cursor from live `cursor-agent models`, Effort `default` | verbatim `### Cursor Agent CLI` subsection | hardcoded slugs or copied `low\|medium\|high` as Cursor effort | `bash tests/contracts.sh /tmp/fixture-t5i` → `FAIL: skills/setup/SKILL.md ### Cursor Agent CLI Discovery subsection no longer matches the approved policy verbatim` line 124 |
| Kiro discovery policy unchanged | existing verbatim Kiro subsection | Cursor edit rewrites the Kiro subsection and still "mentions Kiro" | `bash tests/contracts.sh` (candidate) → pass; Kiro subsection check line 122 still green |
| README Cursor path is marketplace add + `/add-plugin`, not `npx skills` | Cursor README section needles + reject `npx skills` in that section | Cursor section says `npx skills add … --agent cursor` | `bash tests/contracts.sh /tmp/fixture-t5c` → `FAIL: README.md Cursor Agent CLI section must not document npx skills`; `FAIL: README.md Cursor Agent CLI section is missing: plugin marketplace add` lines 138-141 |
| Kiro README path remains `npx skills` | existing Kiro README needles | Cursor work deletes or genericizes the Kiro section | `bash tests/contracts.sh /tmp/fixture-t5h` → `FAIL: README.md is missing a ### Kiro CLI section` and 9 missing-needle FAILs lines 132-137 |
| Bug form offers exact `Cursor Agent CLI` | harness dropdown loop | option `Cursor` or `cursor-agent` | `bash tests/contracts.sh` (candidate without bug form change) → `FAIL: bug_report.yml harness dropdown is missing: Cursor Agent CLI` line 191 |
| Headless `cursor-agent -p`/`--print` is forbidden | verbatim AGENTS.md dispatch sentence | intro lists Cursor; forbid list is the pre-change sentence | `bash tests/contracts.sh /tmp/fixture-t5d` → `FAIL: AGENTS.md headless-forbidden dispatch sentence no longer matches the approved policy verbatim` line 128 |
| Permission default is `--force`; other harness flags remain | grep includes `--force` and the existing three flags | replaces `--dangerously-skip-permissions` with `--force`, or names `--yolo`/`--trust` | `bash tests/contracts.sh /tmp/fixture-t5e` → `FAIL: AGENTS.md does not name a permission default for each hand-composed-argv harness` line 66 |
| Versioned manifests are both `0.15.0` | existing version equality check, updated to `0.15.0` | only `.claude-plugin/plugin.json` bumped | `bash tests/contracts.sh` (before task 4) → `FAIL: manifest versions must both be 0.15.0 (claude=0.14.1 codex=0.14.1)` line 24 |
| `tests/contracts.sh` stays ≤ 250 lines | existing `wc -l` closure gate | new Cursor checks pasted as a second Kiro block at 251+ lines | `test "$(wc -l < /tmp/fixture-t5f/tests/contracts.sh)" -le 250` → FAIL: wc -l is 252 > 250 |
