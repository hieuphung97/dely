...
| --- | --- | --- | --- |
| Cursor sidecar shares `./skills/`, no third version | `tests/contracts.sh` on `.cursor-plugin/plugin.json` | `"skills": "./.cursor/skills/"` or a `"version"` field while README claims support | |
| Root Antigravity/Grok manifest unchanged | existing root `.name == "dely"` check; live `agy plugin validate .` and `grok plugin validate .` still ok | root `plugin.json` gained `$schema` or extra keys | |
| Setup discovers Cursor from live `cursor-agent models`, Effort `default` | verbatim `### Cursor Agent CLI` subsection | hardcoded slugs or copied `low\|medium\|high` as Cursor effort | |
| Kiro discovery policy unchanged | existing verbatim Kiro subsection | Cursor edit rewrites the Kiro subsection and still "mentions Kiro" | |
| README Cursor path is marketplace add + `/add-plugin`, not `npx skills` | Cursor README section needles + reject `npx skills` in that section | Cursor section says `npx skills add … --agent cursor` | |
| Kiro README path remains `npx skills` | existing Kiro README needles | Cursor work deletes or genericizes the Kiro section | |
| Bug form offers exact `Cursor Agent CLI` | harness dropdown loop | option `Cursor` or `cursor-agent` | |
| Headless `cursor-agent -p`/`--print` is forbidden | verbatim AGENTS.md dispatch sentence | intro lists Cursor; forbid list is the pre-change sentence | |
| Permission default is `--force`; other harness flags remain | grep includes `--force` and the existing three flags | replaces `--dangerously-skip-permissions` with `--force`, or names `--yolo`/`--trust` | |
| Versioned manifests are both `0.15.0` | existing version equality check, updated to `0.15.0` | only `.claude-plugin/plugin.json` bumped | |
| `tests/contracts.sh` stays ≤ 250 lines | existing `wc -l` closure gate | new Cursor checks pasted as a second Kiro block at 251+ lines | |
