...
```bash
ruby -ryaml -e 'YAML.safe_load(File.read(ARGV.fetch(0)))' .github/workflows/contracts.yml
...
| --- | --- | --- | --- |
| GitHub-recognized community policies and forms | `bash tests/contracts.sh`, Ruby YAML parse, and post-merge community profile | A valid issue form has `name` and `body` but no `description`, so GitHub will not recognize it | `FIXTURE=$(mktemp -d /tmp/dely-task1-remediation.XXXXXX)` resolved to `/tmp/dely-task1-remediation.t2CiVv`; `git archive 2395ca4 \| tar -x -C "$FIXTURE"` populated a complete candidate copy; `ruby -ryaml -e 'd = YAML.safe_load(File.read(ARGV[0])); d.delete("description"); File.write(ARGV[0], d.to_yaml)' "$FIXTURE/.github/ISSUE_TEMPLATE/bug_report.yml"` removed only the top-level `description` key, leaving `feature_request.yml` untouched and valid. `bash tests/contracts.sh "$FIXTURE"; echo "EXIT:$?"` (no pipeline masking the exit status) printed exactly `FAIL: .github/ISSUE_TEMPLATE/bug_report.yml is not a valid GitHub issue form: missing/invalid description` and `EXIT:1`, with no other failure line — isolating the rejection to the missing `description` key. After writing the real `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `SECURITY.md`, both issue forms, and the pull-request template, `bash tests/contracts.sh` on the real repository (commit `2395ca4`) exited 0. |
| Canonical MIT grant and synchronized 0.13.0 manifests | Canonical-license `diff`, `jq`, and GitHub license detection on the pushed ref | `LICENSE` exists but contains only the word `MIT` | `FIXTURE=$(mktemp -d /tmp/dely-task2-remediation.XXXXXX)` resolved to `/tmp/dely-task2-remediation.r4OS4s`; `git archive 8619cf7 \| tar -x -C "$FIXTURE"` populated a complete candidate copy from baseline commit `8619cf73a3829e3a684bc430c2048f2b52fdba83`. Each fixture manifest was set to `0.13.0` with an explicit replayable command: `jq '.version="0.13.0"' "$FIXTURE/.claude-plugin/plugin.json" > "$FIXTURE/.claude-plugin/plugin.json.tmp" && mv "$FIXTURE/.claude-plugin/plugin.json.tmp" "$FIXTURE/.claude-plugin/plugin.json"` and `jq '.version="0.13.0"' "$FIXTURE/.codex-plugin/plugin.json" > "$FIXTURE/.codex-plugin/plugin.json.tmp" && mv "$FIXTURE/.codex-plugin/plugin.json.tmp" "$FIXTURE/.codex-plugin/plugin.json"`. The fixture's `tests/contracts.sh` was copied from the candidate (`cp tests/contracts.sh "$FIXTURE/tests/contracts.sh"`) so the instrument under test matches the reviewed candidate. `printf 'MIT' > "$FIXTURE/LICENSE"` wrote a present-but-wrong license naming only the word `MIT`. `bash tests/contracts.sh "$FIXTURE"; echo "EXIT:$?"` (no pipeline masking the exit status) printed exactly `FAIL: LICENSE is not the canonical MIT text for 2026 Hieu Phung` and `EXIT:1`, with no other failure line — confirming exactly one canonical-license failure and no unrelated failures. On the real repository after writing the canonical `LICENSE` (via `gh api licenses/mit --jq .body` with `[year]`→`2026`, `[fullname]`→`Hieu Phung`) and bumping both manifests to `0.13.0`, `bash tests/contracts.sh` exited 0. |
| Verified self-service README path | Harness `--help` surfaces, both validators, and remote-ref probe | README uses plausible `codex plugin install`; `codex plugin install --help` exits non-zero because the current CLI does not expose it | `codex plugin install --help` on the locally checked Codex CLI 0.149.1 printed `error: unrecognized subcommand 'install' / tip: a similar subcommand exists: 'list'` and exited 2. The README documents `codex plugin add dely@dely` (install from a configured marketplace snapshot) instead, and does not reference `codex plugin install`. |
| One exact CI entry point and green exact-HEAD check | Workflow parse, fixture contract test, local gates, and pull-request check run | Workflow exists but calls `bash tests/missing.sh` | `FIX2=$(mktemp -d /tmp/dely-task3-remediation-entrypoint.XXXXXX)` resolved to `/tmp/dely-task3-remediation-entrypoint.D8WAJA`; `git archive acdc337 \| tar -x -C "$FIX2"` populated a complete candidate copy from accepted commit `acdc337`. The fixture's `tests/contracts.sh` and `.github/workflows/contracts.yml` were copied from the candidate (`cp tests/contracts.sh "$FIX2/tests/contracts.sh"`; `cp .github/workflows/contracts.yml "$FIX2/.github/workflows/contracts.yml"`) so the instrument under test matches the reviewed candidate. The literal replayable Ruby mutation `ruby -ryaml -e 'd = YAML.safe_load(File.read(ARGV[0])); d["jobs"]["contracts"]["steps"].each { |s| s["run"] = "bash tests/missing.sh" if s["run"] == "bash tests/contracts.sh" }; File.write(ARGV[0], d.to_yaml)' "$FIX2/.github/workflows/contracts.yml"` changed only the fixture workflow's `contracts` job step from `run: bash tests/contracts.sh` to `run: bash tests/missing.sh`, leaving every other step untouched. `bash tests/contracts.sh "$FIX2"; echo "EXIT:$?"` (no pipeline masking the exit status) printed exactly `FAIL: /tmp/dely-task3-remediation-entrypoint.D8WAJA/.github/workflows/contracts.yml missing-run-lines` and `EXIT:1`, with no other failure line — isolating the rejection to the missing exact `bash tests/contracts.sh` run line. Two further present, parseable fixtures were also reproduced and each rejected with one precise failure and no other: `name: wrong-contracts` (only the top-level `name` field mutated) printed exactly `FAIL: .../contracts.yml bad-name` and exited 1; the decoy `run: "bash tests/missing.sh # bash tests/contracts.sh"` (the real entry point mutated but a matching comment appended, defeating a substring check) printed exactly `FAIL: .../contracts.yml missing-run-lines` and exited 1, because the exact-line match does not treat the comment suffix as the required line. On the real repository, `bash tests/contracts.sh` against the accepted `.github/workflows/contracts.yml` exited 0. All fixture directories were removed after the runs, leaving a clean worktree. |
| Solo-maintainer-safe protected branch | GitHub ruleset API assertion | Ruleset exists but requires one approval or omits `contracts` | `RULESET_DIR=$(mktemp -d /tmp/dely-task3-ruleset.XXXXXX)` resolved to `/tmp/dely-task3-ruleset.q7A2gD`; a non-mutating, present, parseable fixture was created with the literal command `jq -n '{enforcement: "active", rules: [{type: "pull_request", parameters: {required_approving_review_count: 1}}, {type: "required_status_checks", parameters: {required_status_checks: [{context: "contracts"}]}}, {type: "required_conversation_resolution"}, {type: "deletion"}, {type: "non_fast_forward"}]}' > "$RULESET_DIR/ruleset.json"`, representing an active ruleset with the `contracts` status check, conversation resolution, deletion, and non-fast-forward rules all present but `required_approving_review_count: 1` instead of the required `0`. Running this row's exact acceptance predicate — `jq -e '.enforcement == "active" and any(.rules[]; .type == "pull_request" and .parameters.required_approving_review_count == 0) and any(.rules[]; .type == "required_status_checks" and any(.parameters.required_status_checks[]; .context == "contracts")) and any(.rules[]; .type == "required_conversation_resolution") and any(.rules[]; .type == "deletion") and any(.rules[]; .type == "non_fast_forward")' "$RULESET_DIR/ruleset.json"` — printed exactly `false` and exited 1 (unmasked, no pipeline), isolating the rejection to the one-approval requirement. No forge state was read or mutated for this row; Control creates and verifies the real active ruleset during release, after the `contracts` check has succeeded on the pushed candidate, per this plan's execution envelope. |
| Private vulnerability intake | GitHub private-vulnerability-reporting API assertion | `SECURITY.md` names private reporting while the forge setting remains disabled | Read-only, non-mutating probe against the live repository: `gh api repos/hieuphung97/dely/private-vulnerability-reporting` printed exactly `{"enabled":false}` and exited 0. Piping through the acceptance assertion, `gh api repos/hieuphung97/dely/private-vulnerability-reporting \| jq -e '.enabled == true'` printed exactly `false` and exited 1 — confirming the forge setting is currently disabled while `SECURITY.md` already documents private reporting as primary. Control enables private vulnerability reporting during release, at the same boundary as the ruleset, per this plan's execution envelope. |
