...
| One exact CI entry point and green exact-HEAD check | Workflow parse, fixture contract test, local gates, and pull-request check run | Workflow exists but calls `bash tests/missing.sh` | `FIXTURE=$(mktemp -d /tmp/dely-task3-remediation.XXXXXX)` resolved to `/tmp/dely-task3-remediation.2AGSgp`; `git archive 62ce230 \| tar -x -C "$FIXTURE"` populated a complete candidate copy from baseline commit `62ce230`. The fixture's `tests/contracts.sh` and `.github/workflows/contracts.yml` were copied from the candidate (`cp tests/contracts.sh "$FIXTURE/tests/contracts.sh"`; `cp .github/workflows/contracts.yml "$FIXTURE/.github/workflows/contracts.yml"`) so the instrument under test matches the reviewed candidate. A Ruby YAML edit changed the fixture workflow's `contracts` job step from `run: bash tests/contracts.sh` to `run: bash tests/missing.sh`, leaving every other step untouched. `bash tests/contracts.sh "$FIXTURE"; echo "EXIT:$?"` (no pipeline masking the exit status) printed exactly `FAIL: /tmp/dely-task3-remediation.2AGSgp/.github/workflows/contracts.yml does-not-call-tests/contracts.sh` and `EXIT:1`, with no other failure line — isolating the rejection to the wrong CI entry point. On the real repository after writing the real `.github/workflows/contracts.yml`, `bash tests/contracts.sh` exited 0. The fixture directory was removed after the run, leaving a clean worktree. |
| Solo-maintainer-safe protected branch | GitHub ruleset API assertion | Ruleset exists but requires one approval or omits `contracts` | Read-only, non-mutating probes against the live repository before Control creates the ruleset: `gh api repos/hieuphung97/dely/rulesets --jq '.[] \| select(.name == "Protect main") \| .id'` printed nothing (no ruleset named "Protect main" exists yet), so `ruleset_id=""`. `gh api "repos/hieuphung97/dely/rulesets/"` then printed exactly `{"message":"Not Found","documentation_url":"https://docs.github.com/rest","status":"404"}` and the `gh` invocation exited 1 with `gh: Not Found (HTTP 404)` on stderr. This is the pre-ruleset observed-red state; Control creates and verifies the active ruleset during release, after the `contracts` check has succeeded on the pushed candidate, per this plan's execution envelope. |
