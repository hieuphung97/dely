...
| Canonical MIT grant and synchronized 0.13.0 manifests | Canonical-license `diff`, `jq`, and GitHub license detection on the pushed ref | `LICENSE` exists but contains only the word `MIT` | `FIXTURE=$(mktemp -d .../scratchpad/dely-task2.XXXXXX)` resolved to `.../dely-task2.lcLZkN`; `git archive HEAD \| tar -x -C "$FIXTURE"` populated a complete candidate copy at baseline `8619cf73a3829e3a684bc430c2048f2b52fdba83`; both fixture manifests were set to `0.13.0` via `jq '.version="0.13.0"'`; `printf 'MIT' > "$FIXTURE/LICENSE"` wrote a present-but-wrong license naming only the word `MIT`. `bash tests/contracts.sh "$FIXTURE"; echo "EXIT:$?"` printed exactly `FAIL: LICENSE is not the canonical MIT text for 2026 Hieu Phung` and `EXIT:1`, with no other failure line. On the real repository after writing the canonical `LICENSE` (via `gh api licenses/mit --jq .body` with `[year]`→`2026`, `[fullname]`→`Hieu Phung`) and bumping both manifests to `0.13.0`, `bash tests/contracts.sh` exited 0. |
