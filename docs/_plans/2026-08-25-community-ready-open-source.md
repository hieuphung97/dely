...
| --- | --- | --- | --- |
| GitHub-recognized community policies and forms | `bash tests/contracts.sh`, Ruby YAML parse, and post-merge community profile | A valid issue form has `name` and `body` but no `description`, so GitHub will not recognize it | `bash tests/contracts.sh /path/to/fixture-red` against a controlled fixture (all six community artifacts present; `bug_report.yml` present and parseable but its top-level `description` key omitted, `feature_request.yml` valid) failed with `FAIL: .github/ISSUE_TEMPLATE/bug_report.yml is not a valid GitHub issue form: missing/invalid description` (exit 1), while `feature_request.yml` raised no failure — isolating the rejection to the missing `description` key, not to absence or unrelated fixture gaps. After writing the real `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `SECURITY.md`, both issue forms, and the pull-request template, `bash tests/contracts.sh` on the real repository exited 0. |
