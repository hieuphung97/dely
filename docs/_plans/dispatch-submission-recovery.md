...
| --- | --- | --- | --- |
| Recovery is conditional, read-first and one-shot | `bash tests/dispatch-submission-contract.sh` | A syntactically valid skill says to send Enter immediately after every `input_accepted` receipt, without reading the TUI | Fixture `enter-immediately.md` (after every `input_accepted`, send Enter immediately; do not read the TUI). Same checker: `FAIL: launch/wait skips the TUI read` and `FAIL: launch/wait sends Enter immediately`. Observed before the shipped skill passed. |
| The read cannot be replaced by a rendered-prompt guess | `bash tests/dispatch-submission-contract.sh` | A syntactically valid skill waits 90 seconds, treats a rendered prompt as proof, and sends Enter without confirming it remains in the input box | Fixture `rendered-prompt.md` (90s wait; treat a rendered copy as proof; no input-box confirm). Same checker: `FAIL: launch/wait does not read the TUI on the no-signal path`, `FAIL: launch/wait does not require the task still pending`, `FAIL: launch/wait treats a rendered prompt as submission proof`. |
| Both install surfaces identify the changed contract | `bash tests/managed-block-contract.sh` and two direct `jq -e` checks | A complete copied package keeps the Codex manifest at `0.9.1` while Claude reports `0.10.0` | Tree with Claude `0.10.0` and Codex `0.9.1`. `bash tests/managed-block-contract.sh` → `FAIL: plugin manifest versions differ: claude=0.10.0 codex=0.9.1`. `jq -e '.version == "0.10.0"' .claude-plugin/plugin.json` → `true` (exit 0). Same jq on `.codex-plugin/plugin.json` → `false` (exit 1). |
| Historical scope no longer says the race is fresh-terminal-only | Human reads the exact `docs/decisions.md` diff; no mechanical instrument exists | The new rule is added while the earlier decision still says the race did not recur on an existing terminal | Human reads the diff; location is `docs/decisions.md` section "Orca owns dispatch and observation; the checkout stays shared": the sentence "It did not recur when the terminal already existed." is replaced by **Amended 2026-08-22** stating later deliveries reproduced the unsubmitted draft on an existing terminal. |
