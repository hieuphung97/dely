...
| --- | --- | --- | --- |
| The skill names baseline-red as insufficient | `bash tests/plan-template-shape.sh`, skill case | A `SKILL.md` that adds the counterexample rule only to its implement section and leaves Acceptance untouched — it contains the word, reads as compliant, and still lets a designer write `Yes` | red: `~/.delivery-journal/01a02482-09d3-7a83-ad4d-d2cb4b2d04da/20260821T133553Z-1710-23915.json` (`bash tests/plan-template-shape.sh` on that copy) |
| The template carries the four named columns | same test, header-row case | A template whose header row is the three old columns while a paragraph beneath it describes counterexamples in prose — a word-grep passes it | red: `~/.delivery-journal/01a02482-09d3-7a83-ad4d-d2cb4b2d04da/20260821T133610Z-2074-16781.json` |
| The check refuses a renamed column | same test, cases 3 and 4 | A check that counts four pipe-separated cells without reading their names, so `Requirement \| Instrument \| Notes \| Evidence` passes | red: `~/.delivery-journal/01a02482-09d3-7a83-ad4d-d2cb4b2d04da/20260821T133622Z-2305-12490.json` — naive four-cell count passed that header; this check rejected it |
| The check is permissive about extra columns | same test, case 7 | A check asserting the header equals exactly four columns, which would break any project adding an owner column | red: `~/.delivery-journal/01a02482-09d3-7a83-ad4d-d2cb4b2d04da/20260821T133633Z-2510-12316.json` — exact-four-column check rejected a five-column header; this check passed it |
| Both manifests declare the bumped version | same test, manifest case | Bumping one manifest and not the other, which a single-file assertion passes | red: `~/.delivery-journal/01a02482-09d3-7a83-ad4d-d2cb4b2d04da/20260821T133641Z-2733-2573.json` |
| The gate list runs the new instrument | `AGENTS.md` gate list, read by review | A test file added to the repository and never named in `AGENTS.md`, which every existing gate passes because nothing runs it | red: `~/.delivery-journal/01a02482-09d3-7a83-ad4d-d2cb4b2d04da/20260821T133650Z-2924-27898.json` — grep for `tests/plan-template-shape.sh` failed on a copy of `AGENTS.md` with those lines removed; the live file names it in `bash -n` and as its own gate |
